...
The backend is designed to run on Vercel: `api/index.js` exports the Express app (`server/app.js`) as a single serverless function, reached via a `vercel.json` rewrite of every `/api/(.*)` request to `/api`, and the database is **Vercel Postgres** (via `@vercel/postgres`) rather than a local file — see `.claude/commands/deploy_vercel.md` for the deploy flow. The project previously used a local SQLite file (Node's built-in `node:sqlite`); that was replaced when deploying to Vercel, since serverless functions have no persistent disk to keep a SQLite file on.

The frontend can *also* be deployed separately as a static site to GitHub Pages, with the API staying on Vercel — see `.claude/commands/deploy_github_page.md`. That build mode (`vite build --mode gh-pages`) calls the API cross-origin, which is why `src/lib/api.ts`, `server/app.js` (CORS via `ALLOWED_ORIGINS`), and `server/session.js` (`SameSite=None` cookie in production) all support an out-of-band frontend origin — see those files' entries below.
...
- `npm run build:gh-pages` / `npm run deploy:gh-pages` — build the frontend with `base: '/claude_code_treasure_game/'` and `VITE_API_BASE_URL` pointed at the Vercel API (via `.env.gh-pages`), then push `build/` to the `gh-pages` branch
- `/deploy_github_page` — custom command (`.claude/commands/deploy_github_page.md`) that runs the above and reports the live GitHub Pages URL
...
- **`src/lib/api.ts`**: thin `fetch` wrapper for all `/api/*` calls (`credentials: 'include'` so the session cookie is sent). Requests go to `import.meta.env.VITE_API_BASE_URL ?? '/api'` — same-origin `/api` on Vercel and in local dev, or an absolute URL when set (only `.env.gh-pages` sets this, for the GitHub Pages build).
...
- **`api/index.js`**: the Vercel serverless function entry — `module.exports = require('../server/app')`.
- **`vercel.json`**: `{ buildCommand, outputDirectory: "build", rewrites: [{ source: "/api/(.*)", destination: "/api" }] }`. The rewrite is required — Vercel's `[...path]` file-system catch-all convention (tried first) only matched single-segment `/api/*` paths in production testing; nested paths like `/api/auth/me` 404'd at the platform level before reaching the function (visible via the `x-vercel-error: NOT_FOUND` response header, vs. `x-powered-by: Express` for requests that got through). The rewrite forwards every `/api/*` request to the one function with `req.url` left as the original path, so Express's own `/api/auth`/`/api/scores` mounts match unchanged.
...
- **`server/session.js`**: session mechanics — an opaque random token stored in the `sessions` table, set as an httpOnly `session_token` cookie (7-day expiry, `secure` and `sameSite: 'none'` when `NODE_ENV === 'production'`, otherwise `sameSite: 'lax'`). `'None'` is required for the cookie to be sent on the GitHub Pages deploy's cross-site requests; it's safe for same-origin Vercel traffic too, so there's no separate flag for it. `attachUser` middleware resolves the cookie to `req.user` on every request; `requireAuth` middleware 401s if absent.
